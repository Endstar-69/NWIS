            ...
            >
              ...
              <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-2.5 ${
                ...
                <div className="flex items-center gap-2.5">
                  <span className="font-bold text-sm text-slate-900 dark:text-white">{alert.risk_type.replace('_', ' ')} ALERT</span>
                  <RiskPill level={alert.severity} probability={alert.probability} />
                  <Badge variant={alert.status === 'NEW' ? 'warning' : 'success'}>
                    {alert.status}
                  </Badge>
                </div>
                <div className="text-slate-500 dark:text-slate-400 text-[11px]">
                  Depth: <b className="font-mono text-slate-700 dark:text-slate-300">{alert.depth} m</b> ({alert.formation}) &bull; ID: <span className="font-mono">{alert.alert_id}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-1">
                    {alert.historical_evidence?.slice(0, 2).map((ev, i) => (
                      <div key={i} className={`p-2 rounded-md border text-[11px] ${
                        ...
                        <span className="font-semibold text-sky-600 dark:text-sky-400">{ev.well_name || ev.well_id} @ <span className="font-mono">{ev.depth}m</span>: </span>
                        {ev.mitigation}
                      </div>
                    ))}
                  </div>
              ...
              <div className={`pt-2.5 border-t flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-2.5 rounded-md ${
                ...
                {alert.status === 'NEW' ? (
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    ...
                    <span>Acknowledged by <b>{alert.acknowledged_by}</b></span>
                  </div>
                )}
              </div>
            </div>
